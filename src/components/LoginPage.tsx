  ...
  const handleDirectorSubmit = (e: React.FormEvent) => {
    ...
    setTimeout(() => {
      if (directorMode === 'signup') {
        ...
        // Notify & login (reload will restore active company name state across app components)
        setSuccessMsg('Company & Director registered successfully!');
        setTimeout(() => {
        ...
        <div style={{ width: '100%', maxWidth: '440px', padding: '1rem 0' }}>
          ...
          {activeTab === 'supervisor' && (
            <form onSubmit={handleSupervisorSubmit} style={{ display: 'flex', flexDirection: 'column', gap: '1.1rem' }}>
              ...
              <div>
                ...
                <div style={{ position: 'relative' }}>
                  <input
                    type={showSupCode ? 'text' : 'password'}
                    value={supCode}
                  ...
                  <button
                    type="button"
                    onClick={() => setShowSupCode(!showSupCode)}
                    style={{ position: 'absolute', right: '0.85rem', top: '50%', transform: 'translateY(-50%)', background: 'none', border: 'none', cursor: 'pointer', color: 'var(--text-muted)', display: 'flex', padding: 0 }}
                  >
                ...
                <p style={{ fontSize: '0.68rem', color: 'var(--text-muted)', marginTop: '0.35rem' }}>
                  Enter the secure code assigned to you by the Company Director.
                </p>
              </div>
              ...
              <button type="submit" className="btn btn-primary" disabled={isLoading}
                ...
                {isLoading
                  ...
                  : <><LogIn size={18} /> Sign In as Supervisor</>}
              </button>
            </form>
          )}
          ...
          {activeTab === 'worker' && (
            <form onSubmit={handleWorkerSubmit} style={{ display: 'flex', flexDirection: 'column', gap: '1rem' }}>
              <div>
                <label style={labelStyle}>Employee Code (PIN)</label>
                <div style={{ position: 'relative' }}>
                  <input
                    type="password"
                    pattern="[0-9]*"
                ...
                <p style={{ fontSize: '0.68rem', color: 'var(--text-muted)', marginTop: '0.35rem', textAlign: 'center' }}>
                  Enter your secure 4-to-6 digit PIN to access your personal dashboard.
                </p>
              </div>
              ...
              <button type="submit" className="btn btn-secondary" disabled={isLoading}
                style={{ height: '48px', fontSize: '0.95rem', fontWeight: 700, borderRadius: '12px', display: 'flex', alignItems: 'center', justifyContent: 'center', gap: '0.5rem' }}>
                {isLoading
                  ? <><span className="auth-spinner" /> Signing In...</>
                  : <><Key size={18} /> Access Employee Portal</>}
              </button>
            </form>
          )}
